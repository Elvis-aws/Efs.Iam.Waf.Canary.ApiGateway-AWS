...
Lambda Authorizer
*****************
    - Json web token (Client authenticates through a 3 rd party system)
    - Token is forwarded to API Gateway using headers or request parameters
    - Lambda Authorizer is programmed
    - API Gateway sends token to Lambda
    - Lambda connects with 3 rd party to verify token
